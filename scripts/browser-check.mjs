...
try {
  ...
  check((await page.locator('#eligible-count').innerText()).includes('66 eligible'), 'sample database loads at the project subpath');
  ...
  check(await page.locator('#wheel-entries li').count() === 23, 'wheel initially contains one equal slice per country');
  check(await page.locator('#atlas-countries button').count() === 23, 'atlas lists every country');
  ...
  check(await page.locator('#selection-wheel image').count() === 23, 'country wheel displays actual nation flags');
  ...
  check(await page.locator('#wheel-entries li').count() === 2, 'filtered wheel contains only the two eligible East Asian countries');
  await page.locator('#clear-filters').click();
  ...
  check(await page.locator('.path-row').count() === 66, 'checklist displays every active path');
  ...
  check(await page.locator('#completion-percent').innerText() === '1.5%', 'dashboard computes completion percentage');
  ...
  await waitForSpin(page);
  check(await page.locator('#result-country').innerText() === 'Brazil', 'Path to Country uses the actual path ideology');
  ...
  await page.locator('#application').waitFor({ state: 'visible' });
  check(await page.locator('#completion-percent').innerText() === '1.5%', 'localStorage survives an actual browser restart');
  await page.reload();
