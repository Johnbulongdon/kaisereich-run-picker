...
try {
  ...
  check((await page.locator('#eligible-count').innerText()).includes('90 eligible'), 'sample database loads at the project subpath');
  ...
  check(await page.locator('#wheel-entries li').count() === 109, 'wheel initially contains one equal slice per country');
  check(await page.locator('#atlas-countries button').count() === 109, 'atlas lists every country');
  ...
  check(await page.locator('#selection-wheel image').count() === 109, 'country wheel displays actual nation flags');
  ...
  check(await page.locator('#wheel-entries li').count() === 17, 'country wheel includes all 17 East Asian starting nations');
  await page.locator('#clear-filters').click();
  ...
  check(await page.locator('.path-row').count() === 90, 'checklist displays every active path');
  ...
  check(await page.locator('#completion-percent').innerText() === '1.1%', 'dashboard computes completion percentage');
  ...
  await page.locator('#application').waitFor({ state: 'visible' });
  check(await page.locator('#completion-percent').innerText() === '1.1%', 'localStorage survives an actual browser restart');
  await page.reload();
