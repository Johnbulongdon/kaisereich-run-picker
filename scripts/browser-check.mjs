...
try {
  ...
  check((await page.locator('#eligible-count').innerText()).includes('788 eligible'), 'sample database loads at the project subpath');
  ...
  check(await page.locator('.path-row').count() === 788, 'checklist displays every active path');
  ...
  check(await page.locator('#completion-percent').innerText() === '0.1%', 'dashboard computes completion percentage');
  ...
  await page.locator('#application').waitFor({ state: 'visible' });
  check(await page.locator('#completion-percent').innerText() === '0.1%', 'localStorage survives an actual browser restart');
  await page.reload();
